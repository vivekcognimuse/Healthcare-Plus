...
export default async function NewsPage({ searchParams }) {
  ...
  try {
    ...
    if (response && response.data && Array.isArray(response.data)) {
      ...
      blogs = response.data.filter(
        (blog) => blog && typeof blog === "object" && blog.Title && blog.Slug
      );
      ...
      <div className=" max-w-[1420px] p-side mx-auto min-h-screen  md:pt-[10%]">
        ...
        {featuredBlogs.length > 0 && (
          <div className="grid grid-cols-1 gap-6 mb-16">
            ...
            {Array.from({ length: Math.ceil(featuredBlogs.length / 2) }).map(
              (_, pairIndex) => {
                    ...
                    {secondBlog && (
                      <div
                        ...
                        <Link href={`/news/${secondBlog.Slug}`}>
                          {secondBlog.Thumbnail &&
                          secondBlog.Thumbnail.length > 0 ? (
                            <div className="relative h-64 w-full">
                              <Image
                ...
                <div key={blog.id} className="py-6 border-b border-black/80">
                  <div className="flex justify-between items-center">
                    <Link href={`/news/${blog.Slug}`} className="flex-1">
                      <h3 className="text-xl font-normal hover:text-blue-600 transition-colors">
                        {blog.Title}
                      </h3>
                    </Link>
